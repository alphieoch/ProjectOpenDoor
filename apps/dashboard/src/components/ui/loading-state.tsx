...
const VARIANT_ORB: Record<string, OrbState> = {
  Drive: "working",
  Dots: "composing",
  Orbit: "solving",
  Surfer: "working",
...
export default function LoadingState({
  ...
  label?: string;
  variant?: "Drive" | "Dots" | "Orbit" | "Surfer" | string;
  orbState?: OrbState;
  videoSrc?: string;
  className?: string;
}) {
  ...
  const state = orbState ?? VARIANT_ORB[variant] ?? "working";
  ...
  const elapsedEl = (
    <span className="font-mono text-[12px] tabular-nums" style={{ color: "var(--ink-4)" }}>
      {elapsed}
    </span>
  );
  ...
  if (surfer) {
    return (
      <div role="status" className={`flex w-fit flex-col items-start ${className || ""}`}>
        ...
        <div
          className="mt-2 w-56 overflow-hidden rounded-[10px] shadow-lg border"
          style={{
        ...
        >
          <div className="relative aspect-video w-full" style={{ background: "var(--paper-3)" }}>
            {videoOk ? (
              <video
                src={videoSrc}
                autoPlay
              ...
              <div className="flex h-full w-full flex-col items-center justify-center gap-1.5">
                ...
                <span className="px-3 text-center font-mono text-[10px]" style={{ color: "var(--ink-4)" }}>
                  Video unavailable
                </span>
              </div>
            )}

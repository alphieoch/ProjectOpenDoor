...
export default function PromptBar({
  ...
  /* hand control to the user: stop the demo loop */
  ...
  /* gliding highlight in the @ and / menu */
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    try {
      ...
      // webgl context not supported or disabled
    }
    return () => {
      try {
  ...
  const celebrate = () => {
    if (sweepingRef.current || typeof window === "undefined") return;
  ...
  return (
    <div
      data-promptbar
      className={demo ? `flex min-h-[384px] w-full max-w-lg flex-col justify-end pb-8 ${className || ""}` : `w-full ${className || ""}`}
      ...
      <div ref={composerAnchorRef} className="relative">
        ...
        {menu && (
          <div
            ...
            className="absolute inset-x-0 bottom-full z-20 mb-2 rounded-2xl border p-1 shadow-2xl"
            style={{
          ...
          >
            ...
            {rows.map((row, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => pick(row)}
                  className="relative z-10 flex h-9 w-full items-center gap-2.5 rounded-xl px-2.5 text-left transition-colors"
                >
                  {source && (
                    <span className="flex size-5.5 shrink-0 items-center justify-center" style={{ color: "var(--ink-2)" }}>
                  ...
                  <span className="min-w-0 flex-1 truncate text-[12px]" style={{ color: "var(--ink-3)" }}>
                    {row.desc}
                  </span>
                  {source?.connect && (
                    <span
                      ...
                      className={`shrink-0 text-[12px] font-medium transition-colors ${
                        connected ? "text-emerald-500" : "text-brand hover:underline"
                      }`}
                    >
            ...
            {rows.length === 0 && (
              <div className="flex h-9 items-center px-3 text-[12px]" style={{ color: "var(--ink-3)" }}>
                No matches for “{query}”
              </div>
            )}
            <div className="mt-1 border-t px-3 pt-1.5 pb-1 text-[11px]" style={{ borderColor: "var(--line)", color: "var(--ink-4)" }}>
        ...
        {modelOpen && (
          <div
            ...
            className="absolute bottom-full z-20 mb-2 w-48 rounded-2xl border p-1.5 shadow-2xl"
            style={{
              left: modelMenuLeft,
              borderColor: "var(--line)",
              background: "var(--paper-2)",
          ...
          >
            <span
              aria-hidden
              className="pointer-events-none absolute inset-x-1.5 rounded-xl"
              style={{
                background: "var(--paper-3)",
                top: modelBox?.top ?? 0,
            ...
            {MODELS.map((m, i) => (
              <button
                ...
                className="relative z-10 flex h-8 w-full items-center gap-2 rounded-xl px-2.5 text-left"
              >
                ...
                <span className={`shrink-0 ${m.key === model.key ? "" : "invisible"}`} style={{ color: "var(--brand)" }}>
        ...
        {/* ── composer ───────────────────────────────────── */}
        <div
          className={`relative isolate flex flex-col gap-1.5 overflow-hidden border p-2 shadow-xl transition-all duration-200 focus-within:border-[var(--line-strong)] ${
        ...
        >
          ...
          {attachments.length > 0 && (
            <div className={`flex flex-wrap gap-1.5 pt-0.5 ${pill ? "px-1.5" : "px-1"}`}>
              {attachments.map((file, i) => (
                <span
                  key={`${file}-${i}`}
                  className={`flex h-6.5 items-center gap-1.5 border py-1 pr-1 pl-2 text-[11.5px] shadow-sm ${
                ...
                >
                  ...
                  <button
                    ...
                    className={`-my-1 flex size-5 items-center justify-center transition-colors hover:bg-[var(--line)] ${
          ...
          >
            <button
              ...
              className={`flex size-7 shrink-0 items-center justify-center justify-self-start transition-all duration-150 hover:bg-[var(--paper-3)] active:scale-[0.94] ${
                pill ? "rounded-full" : "rounded-lg"
              } ${plusOpen ? "bg-[var(--paper-3)]" : ""} ${wide ? "col-start-1 row-start-2" : "col-start-1 row-start-1"}`}
              style={{ color: "var(--ink-3)" }}
            >
            ...
            <textarea
              ...
              aria-label="Prompt"
              className={`${tall ? "min-h-[68px]" : "min-h-7"} min-w-0 w-full resize-none bg-transparent px-1 py-1 text-[13px] leading-[18px] outline-none [overflow-wrap:anywhere] ${
            ...
            <button
              ...
              className={`flex h-7 shrink-0 items-center gap-1 px-2 text-[12px] font-medium transition-colors hover:bg-[var(--paper-3)] ${
                pill ? "rounded-full" : "rounded-lg"
            ...
            <button
              ...
              className={`flex size-7 shrink-0 items-center justify-center transition-all duration-150 active:scale-[0.94] ${
                pill ? "rounded-full" : "rounded-lg"
              } ${wide ? "col-start-4 row-start-2" : "col-start-4 row-start-1"}`}
              style={{
                background: listening ? "var(--brand-soft)" : "transparent",
                color: listening ? "var(--brand)" : "var(--ink-3)",
              }}
            >
              {listening ? (
            ...
            <button
              ...
              disabled={!canSend}
              onClick={send}
              className={`flex size-7 shrink-0 items-center justify-center transition-all duration-200 enabled:active:scale-[0.94] disabled:opacity-35 ${
                pill ? "rounded-full" : "rounded-lg"

"use client";

import { useEffect, useState, type ReactNode } from "react";
import ReactMarkdown from "react-markdown";

/* ─────────────────────────────────────────────────────────
 * STREAMING TEXT
 * Words resolve out of blur, inline citations appear in
 * context, then actions and follow-up prompts become usable.
 * ───────────────────────────────────────────────────────── */

const WORD_MS = 55;
const HOLD_MS = 3400;

export type StreamToken = { text: string; cite?: boolean };

export type StreamSource = {
  name: string;
  domain: string;
  href: string;
  image?: string;
};

const DEFAULT_TOKENS: StreamToken[] = [
  ..."Pistachio is your fastest-growing flavor — sales are up 23% this month and margins beat vanilla by 8 points."
    .split(" ")
    .map((text) => ({ text })),
  { text: "", cite: true },
  ..."Stone-fruit flavors are trending in the same range."
    .split(" ")
    .map((text) => ({ text })),
];

const DEFAULT_FOLLOW_UPS = [
  "Which flavors sell best in winter",
  "Compare gelato and soft serve margins",
];

const SOURCE_IMAGES = {
  scoop:
    "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='16' fill='%231f7a5f'/%3E%3Cpath d='M20 36c0 7 5.4 12 12 12s12-5 12-12H20Z' fill='%23fff'/%3E%3Ccircle cx='32' cy='25' r='11' fill='%23bff3dd'/%3E%3Cpath d='M24 24c4-7 13-7 17 0' fill='none' stroke='%231f7a5f' stroke-width='4' stroke-linecap='round'/%3E%3C/svg%3E",
  trends:
    "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='16' fill='%232f6fec'/%3E%3Cpath d='M15 43 27 31l8 7 14-18' fill='none' stroke='%23fff' stroke-width='7' stroke-linecap='round' stroke-linejoin='round'/%3E%3Ccircle cx='49' cy='20' r='5' fill='%23bfe0ff'/%3E%3C/svg%3E",
  market:
    "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='16' fill='%23e56d24'/%3E%3Cpath d='M17 45V25h8v20h-8Zm11 0V16h8v29h-8Zm11 0V30h8v15h-8Z' fill='%23fff'/%3E%3Cpath d='M16 49h32' stroke='%23ffd6b8' stroke-width='4' stroke-linecap='round'/%3E%3C/svg%3E",
};

const DEFAULT_SOURCES: StreamSource[] = [
  { name: "Scoop Data", domain: "scoopdata.io", href: "https://scoopdata.io/", image: SOURCE_IMAGES.scoop },
  { name: "Trends Index", domain: "trends.google.com", href: "https://trends.google.com/trends/", image: SOURCE_IMAGES.trends },
  { name: "Market Basket", domain: "marketbasket.io", href: "https://marketbasket.io/", image: SOURCE_IMAGES.market },
];

function sourceImage(source: StreamSource) {
  return source.image || SOURCE_IMAGES.scoop;
}

function SourceChip({ source }: { source?: StreamSource }) {
  const s = source || DEFAULT_SOURCES[0];
  return (
    <a
      href={s.href}
      target="_blank"
      rel="noreferrer"
      className="ml-0 mr-1 inline-flex h-4.5 translate-y-[-1px] items-center gap-1 rounded-[5px]
        pr-[3px] pl-[3px] align-middle font-mono text-[10.5px] shadow-sm
        transition-colors duration-150 hover:bg-[var(--paper-3)]"
      style={{
        background: "var(--paper-3)",
        color: "var(--ink-2)",
        animation: "pop-in 250ms cubic-bezier(0.23,1,0.32,1) both",
      }}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={sourceImage(s)} alt="" className="source-avatar size-3 rounded-[3px]" />
      <span>{s.domain}</span>
    </a>
  );
}

const ACTION_ICONS: ReactNode[] = [
  <g key="copy"><rect x="9" y="9" width="12" height="12" rx="2.5" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></g>,
  <path key="retry" d="M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6" />,
  <path key="up" d="M7 10v12M15 5.88L14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2a3.13 3.13 0 0 1 3 3.88z" />,
  <path key="down" d="M17 14V2M9 18.12L10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H20a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.76a2 2 0 0 0-1.79 1.11L12 22a3.13 3.13 0 0 1-3-3.88z" />,
];

const ACTION_LABELS = ["Copy response", "Retry", "Helpful", "Not helpful"];

const markdownComponents = {
  h1: ({ children }: any) => <h1 className="text-base font-bold mt-4 mb-2 first:mt-0 text-ink">{children}</h1>,
  h2: ({ children }: any) => <h2 className="text-sm font-bold mt-3 mb-1.5 first:mt-0 text-ink">{children}</h2>,
  h3: ({ children }: any) => <h3 className="text-[13px] font-semibold mt-3 mb-1.5 first:mt-0 text-ink flex items-center gap-1.5">{children}</h3>,
  p: ({ children }: any) => <p className="mb-2.5 last:mb-0 leading-relaxed text-[13.5px] text-ink">{children}</p>,
  ul: ({ children }: any) => <ul className="list-disc pl-5 mb-2.5 space-y-1 text-[13.5px] text-ink">{children}</ul>,
  ol: ({ children }: any) => <ol className="list-decimal pl-5 mb-2.5 space-y-1 text-[13.5px] text-ink">{children}</ol>,
  li: ({ children }: any) => <li className="leading-relaxed text-[13.5px] text-ink">{children}</li>,
  strong: ({ children }: any) => <strong className="font-semibold text-ink">{children}</strong>,
  em: ({ children }: any) => <em className="italic text-ink-2">{children}</em>,
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-2 pl-3 my-2 text-ink-3 italic" style={{ borderColor: "var(--brand)" }}>
      {children}
    </blockquote>
  ),
  code: ({ inline, children }: any) =>
    inline ? (
      <code className="rounded px-1.5 py-0.5 font-mono text-xs bg-[var(--paper-3)] text-ink">{children}</code>
    ) : (
      <code className="block rounded-lg p-3 font-mono text-xs overflow-x-auto bg-[var(--paper-3)] text-ink my-2">{children}</code>
    ),
};

export type StreamingTextProps = {
  text?: string | null;
  tokens?: StreamToken[];
  sources?: StreamSource[];
  followUps?: string[];
  streaming?: boolean;
  loop?: boolean;
  fill?: boolean;
  onCopy?: () => void;
  onRetry?: () => void;
  onVote?: (direction: "up" | "down") => void;
  onFollowUp?: (text: string) => void;
  onDone?: () => void;
};

export default function StreamingText({
  text,
  tokens: tokensProp,
  sources: sourcesProp,
  followUps: followUpsProp,
  streaming = false,
  loop = false,
  fill = false,
  onCopy,
  onRetry,
  onVote,
  onFollowUp,
  onDone,
}: StreamingTextProps) {
  const isLive = text != null || tokensProp != null;
  const tokens = tokensProp || (text ? null : DEFAULT_TOKENS);
  const sources = sourcesProp || (isLive ? [] : DEFAULT_SOURCES);
  const followUps = followUpsProp || (isLive ? [] : DEFAULT_FOLLOW_UPS);

  const [count, setCount] = useState(0);
  const [sourcesOpen, setSourcesOpen] = useState(false);
  const done = isLive ? !streaming && Boolean(text) : count >= (tokens?.length || 0);

  useEffect(() => {
    if (isLive) return;
    const len = tokens?.length || 0;
    if (done && !loop) {
      onDone?.();
      return;
    }
    const t = setTimeout(
      () => setCount((c) => (c >= len ? (loop ? 0 : len) : c + 1)),
      done ? HOLD_MS : WORD_MS,
    );
    return () => clearTimeout(t);
  }, [count, done, loop, isLive, tokens?.length, onDone]);

  const actions = [
    { label: ACTION_LABELS[0], icon: ACTION_ICONS[0], run: onCopy },
    { label: ACTION_LABELS[1], icon: ACTION_ICONS[1], run: onRetry },
    { label: ACTION_LABELS[2], icon: ACTION_ICONS[2], run: () => onVote?.("up") },
    { label: ACTION_LABELS[3], icon: ACTION_ICONS[3], run: () => onVote?.("down") },
  ];

  return (
    <div className={fill ? "w-full" : "min-h-[4rem] w-full max-w-2xl"}>
      {text != null ? (
        <div className="text-[13.5px] leading-relaxed text-ink">
          <ReactMarkdown components={markdownComponents}>{text}</ReactMarkdown>
          {streaming && (
            <span
              className="ml-1 inline-block h-3.5 w-1 translate-y-0.5 rounded-full bg-current animate-pulse"
              style={{ animation: "fade-in 150ms ease-out both" }}
            />
          )}
        </div>
      ) : (
        <p className="text-[13px] leading-relaxed text-ink">
          {tokens?.slice(0, count).map((token, i) =>
            token.cite ? (
              <SourceChip key={i} source={sources[0]} />
            ) : (
              <span
                key={i}
                className="inline [will-change:filter,opacity]"
                style={{ animation: "stream-in 420ms cubic-bezier(0.22,0.61,0.25,1) both" }}
              >
                {token.text}{" "}
              </span>
            ),
          )}
          {!done && (
            <span
              className="ml-0.5 inline-block h-3 w-0.5 translate-y-0.5 rounded-full bg-ink"
              style={{ animation: "fade-in 150ms ease-out both" }}
            />
          )}
        </p>
      )}

      {/* action icons row */}
      <div
        className="mt-2 flex items-center gap-0.5 transition-opacity duration-400"
        style={{ opacity: done ? 1 : 0, pointerEvents: done ? "auto" : "none" }}
      >
        {actions.map((action) => (
          <button
            key={action.label}
            type="button"
            aria-label={action.label}
            onClick={action.run}
            className="flex size-6 items-center justify-center rounded-[6px] text-ink-3
              transition-colors duration-100 hover:bg-[var(--paper-3)] hover:text-ink-2"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              {action.icon}
            </svg>
          </button>
        ))}
        {sources.length > 0 && (
          <button
            type="button"
            aria-expanded={sourcesOpen}
            onClick={() => setSourcesOpen((current) => !current)}
            className="ml-1.5 flex items-center gap-1.5 rounded-[6px] px-1 py-0.5 text-left transition-colors duration-150 hover:bg-[var(--paper-3)]"
          >
            <span className="flex -space-x-1">
              {sources.slice(0, 3).map((source) => (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  key={source.domain}
                  src={sourceImage(source)}
                  alt=""
                  className="source-avatar size-3.5 rounded-full bg-surface shadow-[0_0_0:1.5px_var(--canvas)]"
                />
              ))}
            </span>
            <span className="text-[12px] text-ink-2">
              {sources.length} {sources.length === 1 ? "source" : "sources"}
            </span>
          </button>
        )}
      </div>

      {sources.length > 0 && (
        <div
          className="grid transition-[grid-template-rows,opacity] duration-300"
          style={{
            gridTemplateRows: done && sourcesOpen ? "1fr" : "0fr",
            opacity: done && sourcesOpen ? 1 : 0,
            transitionTimingFunction: "cubic-bezier(0.23, 1, 0.32, 1)",
          }}
        >
          <div className="overflow-hidden">
            <div className="mt-1.5 flex flex-col rounded-[10px] bg-[var(--paper-3)] p-1 shadow-sm border" style={{ borderColor: "var(--line)" }}>
              {sources.map((source) => (
                <a
                  key={source.href}
                  href={source.href}
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-2 rounded-[6px] px-1.5 py-1 text-[12px] text-ink-2 transition-colors duration-150 hover:bg-[var(--paper-2)] hover:text-ink"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={sourceImage(source)} alt="" className="source-avatar size-4 rounded-[4px]" />
                  <span className="animated-underline">{source.name}</span>
                  <span className="ml-auto font-mono text-[10.5px] text-ink-3">{source.domain}</span>
                </a>
              ))}
            </div>
          </div>
        </div>
      )}

      {/* follow-ups */}
      {followUps.length > 0 && (
        <div
          className="mt-2.5 transition-opacity duration-400"
          style={{ opacity: done ? 1 : 0, pointerEvents: done ? "auto" : "none" }}
        >
          <p className="text-[12px] font-medium text-ink-2">Follow-ups</p>
          <div className="mt-0.5 flex flex-col">
            {followUps.map((item, i) => (
              <button
                key={item}
                type="button"
                onClick={() => onFollowUp?.(item)}
                className="-mx-1.5 flex items-center gap-2 rounded-[7px] border-b border-line
                  px-1.5 py-1.5 text-left text-[12.5px] text-ink transition-colors
                  duration-100 hover:bg-[var(--paper-3)]"
                style={
                  done
                    ? { animation: `fade-up 350ms cubic-bezier(0.23,1,0.32,1) ${i * 90}ms both` }
                    : { opacity: 0 }
                }
              >
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--ink-3)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                  <path d="M9 10l-5 5 5 5" />
                  <path d="M20 4v7a4 4 0 0 1-4 4H4" />
                </svg>
                {item}
              </button>
            ))}
          </div>
        </div>
      )}
    </div>
  );
}

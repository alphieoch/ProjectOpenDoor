...
const TONES = ["bg-brand", "bg-amber-500", "bg-emerald-500"];
...
export default function ThinkingState({
  ...
  // If real live reasoning is provided from LLM stream
  const hasLive = liveReasoning !== undefined;
  const working = hasLive ? Boolean(isLive) : stage < 3;
  const autoExpanded = hasLive ? Boolean(isLive) : (stage >= 1 && stage < 4);
  const expanded = manualExpanded ?? autoExpanded;
  const visible = hasLive ? 1 : (stage < 2 ? 0 : stage === 2 ? Math.min(2, v.rows.length) : v.rows.length);

  const traceRef = useRef<HTMLDivElement>(null);
  const [lineHeight, setLineHeight] = useState(0);
  useLayoutEffect(() => {
    if (traceRef.current) setLineHeight(traceRef.current.offsetHeight);
  }, [visible, expanded, variant, stage, liveReasoning]);
  ...
  return (
    <div
      key={variant}
      className={`flex w-full max-w-2xl flex-col ${className || ""}`}
      style={{
        minHeight: working || expanded ? (hasLive ? undefined : 140) : undefined,
    ...
    >
      ...
      <button
        ...
        className="-mx-1.5 flex w-fit items-center gap-2 rounded-lg px-2 py-1 transition-colors duration-100 hover:bg-[var(--paper-3)]"
      >
        {working ? (
        ...
        <span role="status" className="contents">
          {working ? (
            ...
            >
              {hasLive ? "Thinking..." : v.active}
            </span>
          ) : (
            <span
              className="text-[13px] font-medium whitespace-nowrap"
              style={{ color: "var(--ink-2)", animation: "fade-in 350ms ease-out both" }}
            >
              {hasLive ? "Thought" : v.done}
            </span>
          )}
        </span>
        <svg
        ...
        <div className="overflow-hidden">
          <div className="relative mt-1 ml-[5px] pl-4">
            ...
            <div ref={traceRef} className="flex flex-col gap-1 py-1">
              {hasLive && liveReasoning ? (
                <div
                  className="whitespace-pre-wrap font-sans text-xs leading-relaxed py-1"
              ...
              {!hasLive && v.query && (
                ...
                >
                  ...
                  <span className="text-[12.5px]" style={{ color: "var(--ink-2)" }}>{v.query}</span>
                </div>
              )}
                    ...
                    <>
                      ...
                      {variant === "Steps" &&
                        (i < visible - 1 || !working ? (
                          ...
                          <span
                            className="size-3 shrink-0 rounded-full border-[1.5px] border-t-current"
                            style={{
                      ...
                      <span
                        ...
                        style={{ color: variant === "Reasoning" ? "var(--ink-2)" : "var(--ink)" }}
                      >
                        {row.primary}
                      </span>
                      {row.secondary && (
                        <span
                          className={`shrink-0 text-[11.5px] ${row.mono ? "font-mono" : ""}`}
                          style={{ color: "var(--ink-3)" }}
                        >
                          {row.secondary}
                        </span>
                  ...
                  if (variant === "Search") {
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={`${rowClass} transition-colors duration-150 hover:bg-[var(--paper-3)]`}
                        style={animation}
                      >
                        {content}
                      </a>
                    ...
                    return (
                      <button
                        ...
                        className={`${rowClass} transition-colors duration-150 ${
                          selected ? "bg-[var(--paper-3)]" : "hover:bg-[var(--paper-3)]"
                        }`}
                        style={animation}
                      >
                        {content}
              ...
              {!hasLive && variant === "Search" && stage >= 3 && (
                <span
                  className="text-[12px]"
                  style={{ color: "var(--ink-3)", animation: "fade-in 300ms ease-out both" }}
                >
                  +7 more
                </span>
              )}

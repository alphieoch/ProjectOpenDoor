...
export interface SmartPasteInputProps {
  ...
  label?: string;
  className?: string;
  /** Slot left of the textarea (attach, etc.). */
  leading?: React.ReactNode;
  /** Slot between the textarea and the send button (mode picker, etc.). */
  trailing?: React.ReactNode;
  /** Extra classes for the rounded composer shell. */
    ...
    <div className={cn("w-full max-w-[720px]", className)}>
      ...
      >
        ...
        <div className="flex items-end gap-2 px-1">
          {leading ? <div className="mb-1 shrink-0">{leading}</div> : null}
          <Textarea
            ref={inputRef}
            rows={1}
            value={text}
          ...
          />
          {trailing ? <div className="mb-1 shrink-0">{trailing}</div> : null}

...
type ComputerPointer = {
  x: number;
  y: number;
  action: "idle" | "move" | "click" | "type" | "scroll";
  at: string;
};
...
type BotSession = {
  ...
  snapshotId: number;
  /** Last mouse position in viewport CSS pixels — same space as /screenshot width/height. */
...
function viewportPoint(x: unknown, y: unknown): { x: number; y: number } {
  const px = typeof x === "number" ? x : Number.NaN;
  const py = typeof y === "number" ? y : Number.NaN;
  if (!Number.isFinite(px) || !Number.isFinite(py)) {
    throw new Error("A click needs an x and a y inside the page, in screenshot CSS pixels.");
  }
  return {
    x: Math.min(Math.max(px, 0), VIEWPORT.width - 1),
    y: Math.min(Math.max(py, 0), VIEWPORT.height - 1),
  };
}
...
async function clickLocator(
  ...
  acting: { timeout: number; signal?: AbortSignal },
) {
  const box = await locator.boundingBox();
  if (box) {
    const x = box.x + box.width / 2;
    const y = box.y + box.height / 2;
    setPointer(session, x, y, "move");
    await target.mouse.move(x, y);
    setPointer(session, x, y, "click");
    await target.mouse.click(x, y);
    return { x: Math.round(x), y: Math.round(y) };
  }
  await locator.click(acting);
  return null;
}

async function clickByText(session: BotSession, target: Page, text: string, acting: { timeout: number; signal?: AbortSignal }) {
  const needle = text.trim();
  if (!needle) throw new Error("A text click needs the visible label, e.g. Accept all.");
  const re = new RegExp(escapeRegExp(needle), "i");
  for (const frame of target.frames()) {
    const candidates = [
      frame.getByRole("button", { name: re }),
      frame.getByRole("link", { name: re }),
      frame.getByRole("checkbox", { name: re }),
      frame.getByText(re),
    ];
    for (const locator of candidates) {
      ...
      return { ...(await clickLocator(session, target, first, acting)), name: needle };
    }
  }
  throw new Error(
    `Nothing visible matches “${needle}”. Call computer_screenshot, then computer_click with x,y in that ${VIEWPORT.width}×${VIEWPORT.height} screenshot, or computer_snapshot for a ref.`,
  );
}
...
async function clickBySelector(
  ...
  if (!trimmed) throw new Error("A selector click needs a CSS selector.");
  for (const frame of target.frames()) {
    const locator = frame.locator(trimmed);
    if ((await locator.count()) === 0) continue;
    const first = locator.first();
    if (!(await first.isVisible().catch(() => false))) continue;
    return clickLocator(session, target, first, acting);
  }
  throw new Error(`Nothing matches selector ${trimmed}.`);
}

async function settlePage(target: Page, ms = 400) {
  try {
    await target.waitForLoadState("domcontentloaded", { timeout: Math.max(ms, 250) });
  } catch {
    /* page may already be idle */
  }
  await target.waitForTimeout(Math.min(Math.max(ms, 0), 15_000));
}
...
serve<StreamData>({
  ...
  async fetch(request, server) {
    ...
    if (HUMAN_INPUT.has(url.pathname) && request.method === "POST") {
      ...
      try {
        const target = await currentPage(botId);
        return json(await performHumanInput(session, target, url.pathname, body ?? {}));
      } catch (error) {
...
const ACTIONS = new Set(["/click", "/move", "/type", "/key", "/scroll", "/wait"]);
...
async function performHumanInput(
  ...
  const at = (): { x: number; y: number } => viewportPoint(body.x, body.y);
...
async function performAction(
  ...
  if (action === "/click") {
    ...
    const hasXy = typeof body.x === "number" && typeof body.y === "number";
    if (ref) {
      const locator = await resolveRef(session, target, ref, expected);
      const at = await clickLocator(session, target, locator, acting);
      await settlePage(target, 300);
      return { action: "click", ref, url: target.url(), ...(at ?? {}) };
    }
    if (text) {
      const hit = await clickByText(session, target, text, acting);
      await settlePage(target, 300);
      return { action: "click", element: { role: "button", name: text }, url: target.url(), ...hit };
    }
    if (selector) {
      const at = await clickBySelector(session, target, selector, acting);
      await settlePage(target, 300);
      return { action: "click", url: target.url(), ...(at ?? {}) };
    }
    if (hasXy) {
      const { x, y } = viewportPoint(body.x, body.y);
      setPointer(session, x, y, "move");
      await target.mouse.move(x, y);
      setPointer(session, x, y, "click");
      await target.mouse.click(x, y);
      await settlePage(target, 300);
      return { action: "click", x, y, url: target.url() };
    }
    throw new Error(
      "A click needs visible text (e.g. Accept all), a CSS selector, a snapshot ref, or x,y in screenshot CSS pixels.",
    );
  }
  ...
  if (action === "/move") {
    ...
    setPointer(session, x, y, "move");
    await target.mouse.move(x, y);
    return { action: "move", x, y, url: target.url() };
  }

  if (action === "/wait") {
    const ms = typeof body.ms === "number" && Number.isFinite(body.ms) ? body.ms : 800;
    await settlePage(target, Math.min(Math.max(ms, 0), 15_000));
    return { action: "wait", url: target.url() };
  }
  ...
  if (action === "/type") {
    ...
    if (!ref) {
      setPointer(session, session.pointer.x, session.pointer.y, "type");
      await target.keyboard.insertText(body.text);
      if (body.submit === true) await target.keyboard.press("Enter");
      return {
        action: "type",
        characters: body.text.length,

---
title: "Production Readiness Handoff"
description: "Use this page as the handoff brief for the next agent that continues production hardening."
---
...
- [x] Toolchain: Bun + Node on this Mac; Cloud Agents use `.cursor/environment.json`.
- [ ] Typecheck passes in GitHub Actions (`.github/workflows/ci.yml`).
- [ ] Dashboard build passes.
- [ ] Lint passes or lint configuration is fixed.
- [ ] Homepage responsive QA completed.
- [x] Signup opens signup mode from `/signup`.
- [x] New signup redirects to `/onboarding`.
- [x] Existing login redirects to `/dashboard`.
- [x] `/onboarding` requires auth.
- [x] Onboarding progress is persisted on `organizations.metadata.onboarding_checklist`.
- [x] Signup/login rate limiting is in place.
- [x] Email/password validation is hardened (normalize + 10-char password).
- [ ] Secrets exposed outside the repo are rotated.
- [ ] Stripe webhooks are tested.
- [x] Signup credits cannot be granted twice (`signupCreditGranted`).
- [x] Cloud SQL migrations through 0043 are confirmed (not SokoHut / J&L Supabase).

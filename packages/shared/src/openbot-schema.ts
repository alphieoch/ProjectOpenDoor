/**
 * Vendored from CopilotKit/openbot `server/src/computer/schema.ts` (MIT).
...
export type ComputerPointer = {
  /** Viewport CSS pixels — the same space as `width`/`height` and computer_click x,y. */
  x: number;
  y: number;
...
export type ScreenshotResult = {
  ...
  height: number;
  capturedAt: string;
  /** Last bot (or human) pointer, in the same CSS-pixel space as width/height. */
...
/**
 * Click a snapshot ref, visible text, CSS selector, or screenshot-space x,y.
 * Prefer `text` for labeled buttons (cookie consent). x,y are viewport CSS pixels
 * matching computer_screenshot width/height.
 */
export type ClickInput = Partial<ActionTarget> & {
  text?: string;
  selector?: string;
  x?: number;
  y?: number;
...
export type WaitInput = { ms?: number };
export type TypeInput = Partial<ActionTarget> & {
  text: string;
...
export type ActionResult = {
  action: "click" | "move" | "type" | "key" | "scroll" | "wait";
  ref?: string;
  /**
